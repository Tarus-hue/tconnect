...
class DispatcherPanel extends React.Component {
    ...
    handleAutoRequest(trip) {
        if(trip.status == 'SEARCHING' && trip.current_provider_id != 0) {
            ...
            $('.notification').remove();
            $('.container-fluid').first().before('<div class="alert alert-danger notification"><button type="button" class="close" data-dismiss="alert">Ã—</button><p style="margin-top:10px;">Ride is auto assigned. You cannot manually assign drivers.</p></div>');
        ...
        $.ajax({
            ...
            success: function(data) {
                if(typeof data.message !== 'undefined') {
                    $('.container-fluid').first().before('<div class="alert alert-danger notification"><button type="button" class="close" data-dismiss="alert">Ã—</button><p style="margin-top:10px;">'+data.message+'</p></div>');
                    setTimeout(function() { $('.notification').fadeOut('fast', 'linear', function() { $('.notification').delay(5000).remove(); }); }, 5000);
                }
