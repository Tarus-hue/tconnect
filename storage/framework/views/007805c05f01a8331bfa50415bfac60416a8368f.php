		...
		<div class="row no-margin">
		        ...
		        <div class="col-md-6">
		            <dl class="dl-horizontal left-right">
		                <dt><?php echo app('translator')->getFromJson('user.request_id'); ?></dt>
		                <dd><?php echo e($request->id); ?></dd>
		                <dt><?php echo app('translator')->getFromJson('user.time'); ?></dt>
		            ...
		            <div class="user-request-map">
		                ...
		                <div class="from-to row no-margin">
		                    ...
		                    <div class="to">
		                        <h5><?php echo app('translator')->getFromJson('user.to'); ?> <span id="show_location" style="float: right;font-size:14px; display: none;"><a href="#" data-toggle="modal" data-target="#change-location"><span class="fa fa-edit"></span></a></span></h5>
		                        <p><?php echo e($request->d_address); ?></p>
		                    </div>
		                    <div class="type">
		                    	<h5><?php echo app('translator')->getFromJson('user.type'); ?></h5>
		...
		<div class="modal-dialog">
			<div class="modal-content">
				...
				<div class="modal-body">
			        ...
			        <input type="hidden" name="s_latitude" id="origin_latitude" value="<?php echo e($request->s_latitude); ?>">
                    <input type="hidden" name="s_longitude" id="origin_longitude" value="<?php echo e($request->s_longitude); ?>">
                    ...
                    <input type="hidden" name="request_id" id="request_id" value="<?php echo e($request->id); ?>">
...
<script type="text/javascript">
    ...
    $('#change-location').on('shown.bs.modal', function(){
    	$('#origin-input').trigger('blur').val('<?php echo e($request->s_address); ?>');
    	initMap();
    });
    ...
    $('.locchange').on('click',function(){
    	...
    	if(address.length<=0){
    		...
    		$.ajax({
				url: "<?php echo e(url('/extend/trip')); ?>",
				type: 'post',
				data: {
    ...
    <script type="text/jsx">
		...
		var MainComponent = React.createClass({
			...
			getInitialState: function () {
               return {data: [], currency : "<?php echo e(config('constants.currency')); ?>"};
            },
			...
			changeLabel : function(){
				if(this.props.checkState == undefined){
					...
					if(this.props.checkState.status == 'SEARCHING'){
						$("#ride_status").text("<?php echo app('translator')->getFromJson('user.ride.finding_driver'); ?>");
					}else if(this.props.checkState.status == 'STARTED'){
						var provider_name = this.props.checkState.provider.first_name;
						$("#ride_status").text(provider_name+" <?php echo app('translator')->getFromJson('user.ride.accepted_ride'); ?>");
					}else if(this.props.checkState.status == 'ARRIVED'){
						var provider_name = this.props.checkState.provider.first_name;
						$("#ride_status").text(provider_name+" <?php echo app('translator')->getFromJson('user.ride.arrived_ride'); ?>");
					}else if(this.props.checkState.status == 'PICKEDUP'){
						console.log(this.props.checkState.status);
						$("#show_location").show();
						$("#ride_status").text("<?php echo app('translator')->getFromJson('user.ride.onride'); ?>");
					}else if(this.props.checkState.status == 'DROPPED'){
						$("#ride_status").text("<?php echo app('translator')->getFromJson('user.ride.waiting_payment'); ?>");
						$("#show_location").hide();
					}else if(this.props.checkState.status == 'COMPLETED'){
						var provider_name = this.props.checkState.provider.first_name;
						$("#ride_status").text("<?php echo app('translator')->getFromJson('user.ride.rate_and_review'); ?> " +provider_name );
					}
					setTimeout(function(){
						$('.rating').rating();
					},400);
				...
				return (
					<form action="<?php echo e(url('cancel/ride')); ?>" method="POST">
			            ...
			            <div className="status">
			                <h6><?php echo app('translator')->getFromJson('user.status'); ?></h6>
			                <p><?php echo app('translator')->getFromJson('user.ride.finding_driver'); ?></p>
			            </div>

		            	<button type="submit" className="full-primary-btn fare-btn"><?php echo app('translator')->getFromJson('user.ride.cancel_request'); ?></button> 
		            </form>
				);
			}
		});
			...
			render: function(){
				return (
					...
					<input type="hidden" name="request_id" value={this.props.checkState.id} />
						<div className="status">
			                <h6><?php echo app('translator')->getFromJson('user.status'); ?></h6>
			                <p><?php echo app('translator')->getFromJson('user.ride.accepted_ride'); ?></p>
			            </div>
			            <CancelReason/>
		            	<button type="button" className="full-primary-btn" data-toggle="modal" data-target="#cancel-reason"><?php echo app('translator')->getFromJson('user.ride.cancel_request'); ?></button>
		            	<br/>
		            		<h5><strong><?php echo app('translator')->getFromJson('user.ride.ride_details'); ?></strong></h5>
		            	<div className="driver-details">
			            	<dl className="dl-horizontal left-right">
			            		<dt><?php echo app('translator')->getFromJson('user.booking_id'); ?></dt>
				                <dd>{this.props.checkState.booking_id}</dd>
				                <dt><?php echo app('translator')->getFromJson('user.service_type'); ?></dt>
				            	<dd><?php echo e($request->service_type->name); ?></dd>
				                <dt><?php echo app('translator')->getFromJson('user.driver_name'); ?></dt>
				                <dd>{this.props.checkState.provider.first_name} {this.props.checkState.provider.last_name}</dd>
				                <dt><?php echo app('translator')->getFromJson('user.service_number'); ?></dt>
				                <dd>{this.props.checkState.provider_service.service_number}</dd>
				                <dt><?php echo app('translator')->getFromJson('user.service_model'); ?></dt>
				                <dd>{this.props.checkState.provider_service.service_model}</dd>
				                <dt><?php echo app('translator')->getFromJson('user.driver_rating'); ?></dt>
				                <dd>
				                	<div className="rating-outer">
			                            <input type="hidden" value={this.props.checkState.provider.rating} name="rating" className="rating" disabled/>
			                        </div>
				                </dd>
				                <dt><?php echo app('translator')->getFromJson('user.payment_mode'); ?></dt>
				                <dd>{this.props.checkState.payment_mode}</dd>
								<?php if($request->ride_otp == 1): ?>
				                <dt><?php echo app('translator')->getFromJson('user.otp'); ?></dt>
						...
						<div className="modal-dialog">
							<div className="modal-content">
								<div className="modal-header">
									...
									<h4 className="modal-title"><?php echo app('translator')->getFromJson('user.ride.cancel_request'); ?></h4>
								</div>
								<div className="modal-body">
									<select className="form-control" name="cancel_reason" id="cancel_reason">
										<?php if($request->reasons): ?>
											<?php $__currentLoopData = $request->reasons; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $reas): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
												<option value="<?php echo e($reas->reason); ?>"><?php echo e($reas->reason); ?></option>
											<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
										<?php endif; ?>	
										<option value="ot">Others</option>
									</select>	
									<textarea className="form-control <?php if($request->reasons): ?> cancel_hide <?php endif; ?>" id="cancel_text" name="cancel_reason_opt" placeholder="<?php echo app('translator')->getFromJson('user.ride.cancel_reason'); ?>" row="5"></textarea>
								</div>
			...
			render: function(){
				return (
					...
					<input type="hidden" name="request_id" value={this.props.checkState.id} />
						<div className="status">
			                <h6><?php echo app('translator')->getFromJson('user.status'); ?></h6>
			                <p><?php echo app('translator')->getFromJson('user.ride.arrived_ride'); ?></p>
			            </div>
			            <CancelReason/>
		            	<button type="button" className="full-primary-btn" data-toggle="modal" data-target="#cancel-reason"><?php echo app('translator')->getFromJson('user.ride.cancel_request'); ?></button> 
		            	<br/>
		...
		var Pickedup = React.createClass({
			render: function(){
				...
				<div>
					<div className="status">
		                <h6><?php echo app('translator')->getFromJson('user.status'); ?></h6>
		                <p><?php echo app('translator')->getFromJson('user.ride.onride'); ?></p>
		            </div>
		            <br/>
	            		<h5><strong><?php echo app('translator')->getFromJson('user.ride.ride_details'); ?></strong></h5>
	            	<div className="driver-details">
		...
		var DroppedAndCash = React.createClass({
			...
			render: function(){
				...
				<div>
					<div className="status">
		                <h6><?php echo app('translator')->getFromJson('user.status'); ?></h6>
		                <p><?php echo app('translator')->getFromJson('user.ride.dropped_ride'); ?></p>
		            </div>
		            <br/>
		            	<h5><strong><?php echo app('translator')->getFromJson('user.ride.ride_details'); ?></strong></h5>
		            	<dl className="dl-horizontal left-right">
		            	...
		            	<h5><strong><?php echo app('translator')->getFromJson('user.ride.invoice'); ?></strong></h5>
		            	<dl className="dl-horizontal left-right">
                            <dt><?php echo app('translator')->getFromJson('user.ride.distance_travelled'); ?></dt>
                            <dd>{this.props.checkState.distance} <?php echo e(config('constants.distance')); ?></dd>
                            <dt><?php echo app('translator')->getFromJson('user.ride.base_price'); ?></dt>
                            <dd>{this.props.currency}{this.props.checkState.payment.fixed.toFixed(fixval)}</dd>
                            <?php if($request->service_type->calculator == 'MIN'): ?>
                            <dt><?php echo app('translator')->getFromJson('user.ride.minutes_price'); ?></dt>
                            <dd>{this.props.currency}{this.props.checkState.payment.minute.toFixed(fixval)}</dd>
                            <?php endif; ?>
                            <?php if($request->service_type->calculator == 'HOUR'): ?>
                            <dt><?php echo app('translator')->getFromJson('user.ride.hours_price'); ?></dt>
                            ...
                            <dt><?php echo app('translator')->getFromJson('user.ride.distance_price'); ?></dt>
                            ...
                            {this.props.checkState.use_wallet ?
								<span>
								<dt><?php echo app('translator')->getFromJson('user.ride.detection_wallet'); ?></dt>
                            ...
                            {this.props.checkState.payment.discount ?
								<span>
								<dt><?php echo app('translator')->getFromJson('user.ride.promotion_applied'); ?></dt>
                            ...
                            {this.props.checkState.payment.tips ?
								<span>
								<dt><?php echo app('translator')->getFromJson('user.ride.tips'); ?></dt>
                            	<dd>{this.props.currency}{this.props.checkState.payment.tips.toFixed(fixval)}</dd>  
                            	</span>
                            : ''
                            }
                            <dt className="big"><?php echo app('translator')->getFromJson('user.ride.total'); ?></dt>
                            <dd>{this.props.currency}{this.props.checkState.payment.total.toFixed(fixval)}</dd> 
                            <dt className="big"><?php echo app('translator')->getFromJson('user.ride.amount_paid'); ?></dt>
				...
				<div>
					...
					<div className="status">
		            	...
		            	<dl className="dl-horizontal left-right">
                            ...
                            <dt><?php echo app('translator')->getFromJson('user.ride.total'); ?></dt>
			...
			render: function(){
				...
				<?php echo e(csrf_field()); ?></input>
                    <div className="rate-review">
                        <label><?php echo app('translator')->getFromJson('user.ride.rating'); ?></label>
                        <div className="rating-outer">
                            <input type="hidden" value="1" name="rating" className="rating"/>
                        </div>
						<input type="hidden" name="request_id" value={this.props.checkState.id} />
                        <label><?php echo app('translator')->getFromJson('user.ride.comment'); ?></label>
...
<?php echo $__env->make('user.layout.base', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\xampp\htdocs\t_connect\resources\views/user/ride/waiting.blade.php ENDPATH**/ ?>