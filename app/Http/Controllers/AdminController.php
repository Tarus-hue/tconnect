...
class AdminController extends Controller
{
	...
	public function settings_store(Request $request)
	{
		...
		if (!file_exists($config)) {
			...
			$data = "<?php 
				...
				return array(
					...
					'site_email_logo' => '',
					'site_icon' => '',
					'site_copyright' => '&copy; 2020 t-connect',
		...
		if($request->hasFile('site_logo')) {
			...
			$search_text = "'site_icon' => '".config('constants.site_logo')."'";
		...
		if($request->hasFile('site_email_logo')) {
			...
			$search_text = "'site_icon' => '".config('constants.site_email_logo')."'";
	...
	public function settings_payment_store(Request $request)
	{
		...
		$change_content = $file;
			...
			if(!$request->has('daily_target'))
			{
				...
				if(($request->has('cash')==0 && $request->has('card')==0) && $request->has('payumoney')==0 && $request->has('paypal')==0 && $request->has('paypal_adaptive')==0 && $request->has('braintree')==0 && $request->has('paytm')==0){
