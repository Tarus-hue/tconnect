...
class UserApiController extends Controller
{
	...
	public function signup(Request $request)
	{
		...
		$this->validate($request, [
			...
			//User Already Exists
			if($email_case != null) {
				return response()->json(['User already registered!'], 422);  
			}

			if($registeredEmail != null && $registeredMobile != null) {
				//User Already Registerd with same credentials
				if($registeredEmail != null) return response()->json(['User already registered with given email-Id!'], 422);
				else if($registeredMobile != null) return response()->json(['User already registered with given mobile number!'], 422);
			...
			if($registeredEmailNormal != null) return response()->json(['User already registered with given email-Id!'], 422);
            else if($registeredMobileNormal != null) return response()->json(['User already registered with given mobile number!'], 422);
			// QrCode generator
	...
	public function request_status_check() {
		...
		try{
			...
			return response()->json(['data' => $UserRequests , 
				...
				'currency'=>config('constants.currency','Ksh'), 
