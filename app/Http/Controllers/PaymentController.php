...
class PaymentController extends Controller
{
                ...
                switch ($paymentMode) {
                  ...
                  case 'PAYUMONEY':
                    ...
                    if ($request->ajax()) {
                        ...
                        $paramList['merchant_id'] = config('app.payumoney_merchant_id');
                        $paramList['payu_salt'] = config('app.payumoney_salt');
                        ...
                        $hash_string .= config('app.payumoney_salt');
                  ...
                  case 'PAYPAL':
                    ...
                    return $gateway->process([
                        'order' => $random,
                        'item_name' => $random,
                        'item_currency' => config('app.paypal_currency'),
                        'item_quantity' => 1,
                        'amount' => $totalAmount,
                        'description' => 'Test',
                    ]);
                  ...
                  case 'PAYPAL-ADAPTIVE':
                    ...
                    if ($provider->paypal_email != null) {
                        ...
                        $primary_email = config('app.paypal_email', '');
                  ...
                  case 'PAYTM':
                    ...
                    if ($request->ajax()) {
                        ...
                        $callback_url = (config('app.paytm_environment') == 'local') ? 'https://securegw-stage.paytm.in/theia/paytmCallback?ORDER_ID=' . $random : 'https://securegw.paytm.in/theia/paytmCallback?ORDER_ID=' . $random;

                        $paramList["MID"] = config('app.paytm_merchant_id');
                        $paramList["ORDER_ID"] = $random;
                        $paramList["CUST_ID"] = (String) Auth::user()->id;
                        $paramList["INDUSTRY_TYPE_ID"] = config('app.paytm_industry_type');
                        ...
                        $paramList["CHECKSUMHASH"] = PaytmLibrary::getChecksumFromArray($paramList, config('app.paytm_merchant_key'));
    ...
    {
        $random = config('app.booking_prefix').mt_rand(100000, 999999);
