...
<div class="pro-dashboard-head">
    <div class="container">
        ...
        <a href="{{ route('provider.location.index') }}" class="pro-head-link active">@lang('provider.profile.update_location')</a>
...
<script>
    ...
    function initMap() {
        ...
        var userLocation = new google.maps.LatLng(
                @if(Auth::guard('provider')->user()->latitude) {{ Auth::guard('provider')->user()->latitude }} @else 0.514277 @endif, 
                @if(Auth::guard('provider')->user()->longitude) {{ Auth::guard('provider')->user()->longitude }} @else 35.269779 @endif
            );
        ...
        var infowindow = new google.maps.InfoWindow({
            content: '@lang('provider.your_location')',
        });
...
<script src="https://maps.googleapis.com/maps/api/js?key={{ Config::get('app.map_key') }}&libraries=places&callback=initMap&language=@lang('provider.lang')" async defer></script>
@endsection
